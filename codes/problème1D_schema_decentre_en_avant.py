...
print("Le schema de resolution utilise est adpte à un vecteur vitesse negatif")
c=input("Donner la valeur du champ vitesse : ")
c=int(c)
#N nombre de mailles
N=input("Choisir le nombre de mailles desires : ")
N=int(N)
...
while t<Tmax : 
    ...
    figure(1)
    title("Resultat de l'approximation au differents temps consideres'")
    plot(x,u,'-r')
    plot(x,u_exacte,':b')
#Comparaison des courbes aux derniers temps 
figure(2)
title("Comparaison des solutions au temps Tmax")
plot(x,u,'-r',label="solution calcule")
